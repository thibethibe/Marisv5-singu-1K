...
var msg = "";
    switch(handleReply.type) {
        case "choosee": {
            ...
            return api.sendMessage({body: `${msg}`, attachment: await DownLoad('https://i.imgur.com/vr4KGBz.jpg')}, threadID, async () => {
            data.workfishTime = Date.now();
...
module.exports.run = async ({  event, api, handleReply, Currencies, getText }) => {
  const fs = require("fs-extra");
    ...
    if (typeof data !== "undefined" && cooldown - (Date.now() - data.workfishTime) > 0) {

        var time = cooldown - (Date.now() - data.workfishTime),
    ...
    var msg = {
      body: "🐠👽CÂU CÁ TRÊN 4546B👽🐠" +
            ...
            attachment: await DownLoad('https://i.imgur.com/smiolrW.jpg')}
