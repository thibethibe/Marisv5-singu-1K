  ...
  const Canvas = require('canvas');
    ...
    return global.client.handleReply.push({
      ...
      if(!color || color == "no" || color == "No"){
        const lengthchar = (await axios.get(`https://docs-api.jrtxtracy.repl.co/taoanhdep/search?type=id&id=${id}`)).data
      var color_ = lengthchar.colorBg
    } else {
      var color_ = color
    }
    const lengthcharr = (await axios.get(`https://docs-api.jrtxtracy.repl.co/taoanhdep/search?type=id&id=${id}`)).data
let avtAnime = (
