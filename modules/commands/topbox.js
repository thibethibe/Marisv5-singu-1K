...
module.exports.run = async ({
  ...
  if (!allType.includes(g[0]) || !g[0]) return a.sendMessage(`[⚜️]➜ Vui lòng chọn money|level muốn xem`, t, m)
  if (g[1] && isNaN(g[1])) return a.sendMessage(`[⚜️]➜ Phải là 1 con số`, t, m)
  switch (g[0]) {
    case "money": {
      await FOD("money", "m")
      array.sort(VC("m"))
      FO("m")
      msg = `[⚜️]➜ Bảng xếp hạng đại gia trong nhóm ←[⚜️]\n\n`.toUpperCase()
      FF("Money", "m")
      FI("money", "i", "m")
  ...
  case "level": {
    ...
    array.sort(VC("e"))
    FO("e")
    msg = `[⚜️]➜ Bảng xếp hạng level trong nhóm ←[⚜️]\n\n`.toUpperCase()
    FF("Level", "e")
    FI("level", "i", "e")
    a.sendMessage(msg, t, m)
  }
...
function TX(x, k, c) {
  return `\n[👉]➜ ${k} của bạn là ${k == "money" ? CC(c) + "$" : LV(c)} và đang đứng top ${x}\n[⚜️]➜ ${x == 1 ? `Bạn đang vô địch, thật vip pro` : `${x > 20 ? `[⚜️]➜ Bạn đứng top ${x > 20 && x < 30 ? `khá thấp` : x > 30 && x < 50 ? `rất thấp` : `rất rất thấp`}!, cố gắng cày ${k} đi nhé` : x > 10 && x < 20 ? `[⚜️]➜ Sắp lọt top 10 rồi cố gắng lên!!` : `Bạn có trong top 10, cố gắng giữ phong độ nhé!`}`}`
}