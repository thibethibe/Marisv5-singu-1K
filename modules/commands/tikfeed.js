  ...
  const fs = global.nodemodule["fs-extra"];
    switch (handleReply.type) {
        case "choose": {
              ...
              if (isNaN(choose)) return api.sendMessage("[⚜️]➜ Vui lòng nhập 1 con số", threadID);
              ...
              if (choose > lengthData || choose < 1) return api.sendMessage("[⚜️]➜ Lựa chọn không nằm trong danh sách.", threadID)
              ...
              if (fs.statSync(__dirname + `/cache/${event.senderID}.mp4`).size > 26000000) return api.sendMessage('[⚜️]➜ Không thể gửi file vì dung lượng lớn hơn 25MB.', event.threadID, () => unlinkSync(__dirname + `/cache/${event.senderID}.mp4`), event.messageID);
...
const axios = global.nodemodule['axios'];  
if (!args[0]){ return api.sendMessage("[⚜️]➜ Bạn phải nhập username tik tok", event.threadID, event.messageID);}
...
var options = {
  ...
  // }
  // else 
  return api.sendMessage(`[⚜️]➜ Hiện đang có ${lengthData} video và feelings được tìm thấy trên user này\n[⚜️]➜ Vui lòng reply số thứ tự video để xem!!!`, event.threadID, (error, info) => {       
            global.client.handleReply.push({
                type: "choose",
                name: this.config.name,
                author: event.senderID,
