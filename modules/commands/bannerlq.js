...
module.exports.run = async function({ api, args, event, permssion }) {
  ...
  if (!i && event.type == 'message_reply') {
        ...
        i = `https://graph.facebook.com/${event.senderID}/picture?width=512&height=512&access_token=${global.config.tokenFB[0]}`;
    } else {
        if (i.indexOf('http') == -1) {
            i = 'https://' + i;
        }
  ...
  module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
    ...
    if (!fs.existsSync(__dirname +
      ...
      let getfont = (await axios.get(`https://raw.githubusercontent.com/duongcongnam/font/main/ArialUnicodeMS.ttf`, { responseType: "arraybuffer" })).data;
  ...
  if (handleReply.step == 1) {
    ...
    Canvas.registerFont(__dirname + `/cache/ArialUnicodeMS.ttf`, {
      family: "Arial"
    });
