...
if(args[0] == "me"){
	 if (args[1] == "admin") {
	  ...
	  else if(!global.config.ADMINBOT.includes(event.senderID)) api.sendMessage("Quyền hạn lồn biên giới ?", event.threadID, event.messageID)
...
if(!find) {api.sendMessage("Không ném quản trị viên cho bot dùng con cặc ?", event.threadID, event.messageID)}
