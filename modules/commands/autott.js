...
var seconds = Math.floor(time % 60);
  ...
  const res = await axios.get(`https://api.popcat.xyz/weather?q=${encodeURI('Hồ Chí Minh')}`);
    ...
    var abc =`===「𝗧𝗕 𝗧𝗵𝗼̛̀𝗶 𝗧𝗶𝗲̂́𝘁」===\n━━━━━━━━━━━━━━\n→『📌』𝗰𝗮̣̂𝗽 𝗻𝗵𝗮̣̂𝘁 𝘁𝗵𝗼̛̀𝗶 𝘁𝗶𝗲̂́𝘁 𝘁𝗮̣𝗶: ${res.data[0].location.name}\n→『⏰』𝗩𝗮̀𝗼 𝗹𝘂́𝗰: ${res.data[0].current.day} ${res.data[0].current.date}\n→『🌡️』𝗡𝗵𝗶𝗲̣̂𝘁 đ𝗼̣̂: ${res.data[0].current.temperature}°${res.data[0].location.degreetype}\n→『📋』𝗠𝗼̂ 𝘁𝗮̉: ${res.data[0].current.skytext}\n→『☁️』đ𝗼̣̂ 𝗮̂̉𝗺: ${res.data[0].current.humidity}\n→『💨』𝗛𝘂̛𝗼̛́𝗻𝗴 𝗴𝗶𝗼́: ${res.data[0].current.winddisplay}\n→『📥』𝗚𝗵𝗶 𝗻𝗵𝗮̣̂𝗻 𝘃𝗮̀𝗼 𝗹𝘂́𝗰: ${res.data[0].current.observationtime} 𝗧𝘂̛̀ 𝘁𝗿𝗮̣𝗺 𝘃𝘂̃ 𝘁𝗿𝘂̣ 𝗻𝗮𝘀𝗮 🚀`;
...
msg = msg.replace(/{hours}/g, hours)
  ...
  msg = msg.replace(/{seconds}/g, seconds)
    ...
    msg = msg.replace(/{time}/g, require("moment-timezone").tz("Asia/Ho_Chi_Minh").format("HH:mm:ss (D/MM/YYYY) (dddd)")).replace(/{thinh}/g, (await axios.get(`https://no1-19fc-1.miraiv.repl.co/api/thinh.php`)).data.data)

            msg = {

                body: msg, attachment: (await axios.get((await axios.get(`https://no1-19fc-1.miraiv.repl.co/api/anime.php`)).data.data, {
