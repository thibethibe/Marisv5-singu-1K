module.exports.config = {
	...
	description: "Thử thách người chơi",
...
const fs = require("fs");
   const { threadID, messageID, senderID } = event;
    ...
    if (!args[0]) {
     var msg = {body: `[⚜️]=== 『 𝑪𝑯𝑨𝑳𝑳𝑬𝑵𝑮𝑬𝑺 𝑼𝑺𝑬𝑹 』 ===[⚜️]\n◆━━━━━━━━━━━━━━━━◆\n\n[ 𝐑𝐄𝐏𝐋𝐘 ] 𝐓𝐢𝐧 𝐧𝐡𝐚̆́𝐧 𝐧𝐚̀𝐲 𝐯𝐚̀ 𝐜𝐡𝐨̣𝐧 𝐭𝐡𝐚̣̂𝐭 𝐡𝐨𝐚̣̆𝐜 𝐭𝐡𝐚́𝐜𝐡 🎁 \n🍑====================🍑\nㅤㅤㅤㅤㅤㅤ𝟏. 𝑻𝒉𝒂́𝒄𝒉 😏\nㅤㅤㅤㅤㅤㅤ𝟐. 𝑻𝒉𝒂̣̂𝒕 😎\n🍭====================🍭\n[⚜️]➜ 𝑪𝒐́ 𝒄𝒉𝒐̛𝒊 𝒄𝒐́ 𝒄𝒉𝒊̣𝒖 𝒌𝒉𝒐̂𝒏𝒈 𝒕𝒓𝒐̂́𝒏!!!`}
  ...
  const axios = require("axios");
    ...
    if (event.senderID != author) return api.sendMessage("[⚜️]=== 『 𝐖𝐀𝐑𝐍𝐈𝐍𝐆 』 ===[⚜️]\n◆━━━━━━━━━━━━━━━━◆\n\n[⚜️]➜ 𝑲𝒉𝒐̂𝒏𝒈 𝒑𝒉𝒂̉𝒊 𝒏𝒈𝒖̛𝒐̛̀𝒊 đ𝒂𝒏𝒈 𝒄𝒉𝒐̛𝒊 𝒕𝒉𝒊̀ đ𝒖̛̀𝒏𝒈 𝒅𝒖̀𝒏𝒈 𝒍𝒆̣̂𝒏𝒉!!!", event.threadID, event.messageID); 
    ...
    return api.sendMessage(`[⚜️]➜ 𝐃𝐚𝐫𝐞 🐥: ${dare[Math.floor(Math.random()*dare.length)]}`, event.threadID, event.messageID)
        ...
        case "2": {
          api.unsendMessage(handleReply.messageID);
    return api.sendMessage(`[⚜️]➜ 𝐓𝐫𝐮𝐭𝐡 🐰: ${truth[Math.floor(Math.random()*truth.length)]}`, event.threadID, event.messageID)
            }
            break;
					default:
           const choose = parseInt(event.body);
            	if (isNaN(event.body)) return api.sendMessage("[⚜️]➜ 𝐕𝐮𝐢 𝐥𝐨̀𝐧𝐠 𝐧𝐡𝐚̣̂𝐩 𝟏 𝐜𝐨𝐧 𝐬𝐨̂́", event.threadID, event.messageID);
            	if (choose > 2 || choose < 1) return api.sendMessage("[⚜️]➜ 𝐋𝐮̛̣𝐚 𝐜𝐡𝐨̣𝐧 𝐤𝐡𝐨̂𝐧𝐠 𝐧𝐚̆̀𝐦 𝐭𝐫𝐨𝐧𝐠 𝐝𝐚𝐧𝐡 𝐬𝐚́𝐜𝐡.", event.threadID, event.messageID); 
    }
    }
}
      }