...
`,attachment: fs.createReadStream(__dirname + `/noprefix/mv.gif`)}, event.threadID, event.messageID);
  ...
  if (money >= 100) {
  axios.get('https://TPKTAO.last-namename.repl.co/image/vdgai ').then(res => {
...
`,attachment: fs.createReadStream(__dirname + `/noprefix/mv.gif`)}, event.threadID, event.messageID);
  ...
  var money = (await Currencies.getData(event.senderID)).money
  if (money >= 100) {
  axios.get('https://apitoansex.dz1toan.repl.co/gai ').then(res => {
  ...
  var money = (await Currencies.getData(event.senderID)).money
  if (money >= 1000) {
  axios.get('https://apiuk.dz1toan.repl.co/anime').then(res => {
  var video = res.data.url;
  let callback = function () {
          api.sendMessage({
  ...
  var money = (await Currencies.getData(event.senderID)).money
  if (money >= 1000) {
  axios.get('https://TPKTAO.last-namename.repl.co/image/vdanime').then(res => {
  var video = res.data.url;
  let callback = function () {
          api.sendMessage({
            body: `🌸 𝗩𝗶𝗱𝗲𝗼 𝗮𝗻𝗶𝗺𝗲 𝗻𝗲̀\n -100 đô💸`,
