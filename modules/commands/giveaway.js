  ...
  const moment = require("moment");
    ...
    var x = global.data.allCurrenciesID;var out = (msg) => api.sendMessage(msg,event.threadID,event.messageID);
        ...
        await Currencies.increaseMoney(ex, parseInt(ix));
                ...
                for (let exs of rxxi) {
        api.sendMessage(`[⚜️]=== 『 𝐆𝐈𝐕𝐄𝐀𝐖𝐀𝐘 𝐀𝐋𝐋𝐁𝐎𝐗 』 ===[⚜️]\n◆━━━━━━━━━━━━━━━━◆\n\n[⚜️]➜ Vào lúc: ${time}\n[⚜️]➜ Nhóm Bạn Đã Được Admin tặng quà gồm:\n[⚜️]➜ Random nhiều tiền (tự cộng random cho all tv)\n[⚜️]➜ Thính: ${thinh[Math.floor(Math.random() * thinh.length)]}\n[⚜️]➜ Chúc các bạn sử dụng BOT vui vẻ.\n◆━━━━━━━━━━━━━━━━◆\n[⚜️]➜ Lưu ý: Nhớ check money để xem số tiền tặng nhé!!!`,exs,(error,info) => {
              if (error) return;
        });
                }
                 xs = true;
            return api.sendMessage("[⚜️]➜ Đã gửi quà đến all box thành công",event.threadID);                      
           }
        }
    };