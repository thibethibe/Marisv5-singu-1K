...
module.exports.run = async function ({ api, event, args, client,Threads,__GLOBAL, Users, Currencies,getText }) {
   const { senderID, messageID, threadID } = event;
    ...
    switch (args[0]) {
     ...
     return api.sendMessage({body: `── [ Shop ] ──  \n\n💌Danh sách rồng bạn có thể mua\n[💌1].Hàn Phi Tuyết\n[💌2].Bất Tử Điểu\n[💌3].Rồng Rùa\n[💌4].Ngũ Sắc\n[💌5].Hắc Long 35 sao\n[💌6].Huyền Vũ Long\n[💌7].Việt Hải Long Vương\n[💌8].Kỳ Long\n[💌9].Thần Long\n[💌] Hãy reply tin nhắn bot và kèm theo số`, attachment: await this.image('https://imgur.com/ULwGlLx.jpg')}, threadID, (error, info) => {
                global.client.handleReply.push({
                    name: this.config.name,
                    messageID: info.messageID,
                    author: event.senderID,
     ...
     return api.sendMessage({body: `── [ CHỌN ĐẢO THI ĐẤU ] ──  \n\n❤️ Chọn đảo để thi đấu\n[🗺1].Đảo dung nham\n[🗺2].Đảo Tuyết\n[🗺3].Đảo thần tiên\n`, attachment: await this.image('https://imgur.com/varoTlL.jpg')}, threadID, (error, info) => {
        ...
        case 'top': {
            ...
            var msg = '===TOP 3 NGƯỜI CHƠI CÓ LEVEL CAO NHẤT===\n'
            for(var i = 0; i < 3; i++) {
