...
module.exports.run = async ({ event, api, args, }) => {
    ...
    if (global.moduleData.pornList.some(pornURL => urlParsed.host == pornURL)) return api.sendMessage("[⚜️]➜ Trang web bạn nhập không an toàn!!(NSFW PAGE)", event.threadID, event.messageID);
    ...
    try {
        ...
        return api.sendMessage("[⚜️]➜ Không tìm thấy url này, định dạng không đúng ?", event.threadID, event.messageID);
    }
}