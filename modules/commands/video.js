...
module.exports.languages = {
  vi: {
    overSizeAllow: "[⚜️]➜ Không thể gửi file vì dung lượng lớn hơn 25MB.",
    returnError: "[⚜️]➜ Đã xảy ra vấn đề khi đang xử lý request, lỗi: %1",
    cantProcess: "[⚜️]➜ Không thể xử lý yêu cầu của bạn!",
    missingInput: "[⚜️]➜ Phần tìm kiếm không được để trống!",
    returnList:
    ...
    ytdl(handleReply.link[event.body - 1], {quality: 18})
      ...
      .on("close", () => {
        if (
          ...
          return api.sendMessage("[⚜️]➜ Không thể gửi file vì dung lượng lớn hơn 25MB.",
            event.threadID,
            () =>
              unlinkSync(
                __dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`
      ...
      ytdl(args[0])
        ...
        .on("close", () => {
          if (statSync(__dirname + `/cache/${id}.mp4`).size > 26214400)
            return api.sendMessage(
              "[⚜️]➜ Không thể gửi file vì dung lượng lớn hơn 25MB.",
              event.threadID,
    ...
    try {
      ...
      for (const value of results) {
        if (typeof value.id.videoId !== "undefined") {
          ...
          msg += `${num}. ${value.title}\n[⚜️]➜ Thời lượng: ${value.duration_raw}\n[⚜️]➜ Tên kênh: ${video.videoDetails.ownerChannelName}\n\n`;
          let gT = (
            await axios.get(
